...
# Revision:	$Id: Iterator.pm,v 1.1 2009-09-22 18:05:08 ubuntu Exp $
# 
# Contributors:
...
sub next {
	...
	throw ODO::Exception::Runtime(error=> '$self->next_item() did not return an RDFS::List or ODO::Node object')
		unless(	   UNIVERSAL::isa($self->next_item(), 'RDFS::List')
	...
	# If the rest of the list is just a pointer to the URI rdf:nil which is an instance of RDFS::List, spare
	...
	if($rest->subject()->equal($ODO::Ontology::RDFS::Vocabulary::nil)) {
		...
		# The rdf:rest of a list with elements remaining is itself an RDFS::List object
		my $l = RDFS::List->new($rest->subject(), $self->list()->graph());
		throw ODO::Exception::Runtime(error=> 'Could not create RDFS::List object from RDFS::Properties::rest object')	
			unless(UNIVERSAL::isa($l, 'RDFS::List'));
...
sub init {
	...
	throw ODO::Exception::Parameter::Invalid(error=> 'Parameter is not of type RDFS:List')
		unless(UNIVERSAL::isa($config->{'list'}, 'RDFS::List'));
